...
import 'package:grimoire_mtg/scanner/text_scanner.dart';
...
void main() {
  ...
  group('OCR Accuracy Tests - assets/$dataset_path', () {
    ...
    testWidgets('Sprawdź wszystkie obrazy z $dataset_path', (tester) async {
      // 1. Pobierz listę plików z manifestu assetów
      final AssetManifest assetManifest = await AssetManifest.loadFromAssetBundle(rootBundle);
      final List<String> imagePaths = assetManifest
          .listAssets()
          .where((String key) => key.startsWith('assets/$dataset_path/'))
          .toList();

      if (imagePaths.isEmpty) {
        fail('Nie znaleziono żadnych obrazów w assets/$dataset_path/');
      }
