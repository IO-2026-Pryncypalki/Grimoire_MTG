...
void main() {
  ...
  group('OCR Accuracy Tests - assets/easy', () {
    final ocrService = TextScanner(); // Twoja klasa OCR

    testWidgets('Sprawdź wszystkie obrazy z białym tłem', (tester) async {
      // 1. Pobierz listę plików z manifestu assetów
      final AssetManifest assetManifest = await AssetManifest.loadFromAssetBundle(rootBundle);
      final List<String> imagePaths = assetManifest
          .listAssets()
          .where((String key) => key.startsWith('assets/easy/'))
          .toList();

      if (imagePaths.isEmpty) {
        fail("Nie znaleziono żadnych obrazów w assets/easy/");
      }
